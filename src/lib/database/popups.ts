...
export interface ActivePopup {
  ...
  /** 'event' opens an event page inside the app; 'external' opens `link` in the browser. */
 ...
 * audience and 21+ eligibility. Nothing is consumed by fetching: we send
 * `clientReportsViews` so the impression is recorded by `recordPopupView`
 * once a banner is actually on screen, and anything fetched but never
 * displayed comes back on the next fetch.
...
/**
 * Record that a pop-up was actually rendered on screen. Fire-and-forget:
 * failures are logged, never thrown — reporting must never block display.
 * The server treats a repeat for the same user/popup/day as a no-op, so a
 * retry is safe.
 * @param userId - The user's profile document ID
 * @param popupId - The popup document ID that was shown
 */
